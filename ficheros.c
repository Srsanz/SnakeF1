    ...
    do
    {
        ...
        case 1:
            ...
            else
            {
                ...
                superior = comparaPuntuacion(nombre, nueva_puntuacion);
...
int buscaJugador(char* nombre)
{
    ...
    {
        if (strcmp(jugadores[i].nombre, nombre) == 0) //compara la cadena introducida con las existentes 
        {
            return i;
        }
    }
...
#pragma warning(default:4996)
    ...
    char nombre[NOMBREMAX];
    int puntuacion;
    while (fscanf(archivo, "%s %d", nombre, &puntuacion) == 2) 
    {
        Jugador j; 
